...
export default function RoutesPage() {
  ...
  const load = async () => {
    ...
    setOrders(byDate);
    setUnassigned(un.filter((o: Order) => !o.delivery_date || o.delivery_date === dateStr));
  };
  ...
  const grouped = useMemo(() => {
    ...
    orders
      ...
      .forEach((o) => ((map[o.driver_id!] = map[o.driver_id!] || []).push(o)));
  ...
  const orphan = orders.filter((o) => !o.driver_id);
  ...
  const assignTo = async (orderId: number, driverId: number | null) => {
    await fetch("/api/routes/assign", {
      ...
      body: JSON.stringify({
        assignments: [{ order_id: orderId, driver_id: driverId, delivery_date: dateStr }],
      }),
    });
  ...
  return (
    <>
      ...
      {drivers.length === 0 ? (
        <Card>
          <Empty description="Chưa có tài xế. Vào trang Tài xế để thêm." />
        </Card>
      ) : (
